...
前端查詢時只需 SELECT symbol FROM screener_cache WHERE screener_type = '...'，
不再即時跑重型查詢，大幅降低 DB 記憶體壓力。

執行方式：
  python run_screener_cache.py
"""
...
SCREENERS = {
    ...
    "ma20_breakout": """
        ...
        prev5 AS (
            ...
            JOIN (SELECT symbol, date AS latest_date FROM latest) l
    ...
    "gross_margin_rising": """
        WITH ranked AS (
            SELECT symbol, year, quarter, gross_margin,
                   ROW_NUMBER() OVER (
                       PARTITION BY symbol ORDER BY year DESC, quarter DESC
                   ) AS rn
        ...
        SELECT symbol, gm1, gm2, gm3, gm4
        FROM pivoted
        WHERE cnt = 4 AND min_gm >= 30
    """,
    ...
    "operating_margin_rising": """
        ...
        SELECT symbol, om1, om2, om3, om4
        FROM pivoted
        WHERE cnt = 4 AND min_om >= 10
    """,
    ...
    "net_income_outpace_revenue": """
        ...
        recent AS (
            SELECT r.symbol, r.net_income, r.revenue,
    ...
    "revenue_yoy_consecutive": """
        WITH ranked AS (
            SELECT symbol, year_month, revenue_yoy,
                   ROW_NUMBER() OVER (
                       PARTITION BY symbol ORDER BY year_month DESC
                   ) AS rn
        ...
        check_pass AS (
            SELECT symbol, COUNT(*) AS cnt,
                   BOOL_AND(revenue_yoy >= 0) AS all_pass
            FROM ranked WHERE rn <= 3
            GROUP BY symbol
        )
        SELECT symbol FROM check_pass
        WHERE cnt = 3 AND all_pass = TRUE
        ORDER BY symbol
    """,
}

# ── 需要 Python 端過濾的篩選器（SQL 先撈候選，再 Python 嚴格驗證） ─────────────

def filter_gross_margin_rising(rows):
    result = []
    for row in rows:
        sym = row[0]
        gms = [float(row[i+1]) for i in range(4) if row[i+1] is not None]
        if len(gms) < 4:
            continue
        if all(gms[i] > gms[i+1] for i in range(3)):
            result.append(sym)
    return result
...
def filter_operating_margin_rising(rows):
    ...
    for row in rows:
        sym = row[0]
        oms = [float(row[i+1]) for i in range(4) if row[i+1] is not None]
        if len(oms) < 4:
            continue
        if all(oms[i] > oms[i+1] for i in range(3)):
            result.append(sym)
    return result
...
def filter_contract_liabilities_growth(rows):
    ...
    for row in rows:
        sym = row[0]
        cls = [float(row[i+1]) for i in range(4) if row[i+1] is not None]
        if len(cls) < 2:
            continue
        # 條件 B：最新季 QoQ >= 20%
...
def main():
    ...
    for name, sql in SCREENERS.items():
        ...
        try:
            ...
            # 需要 Python 後處理的篩選器
            if name in PYTHON_FILTERS:
                symbols = PYTHON_FILTERS[name](rows)
            else:
            ...
            conn.commit()
            logger.info(f"  ✅ {name}：{len(symbols)} 支")
            total_success += 1

        except (psycopg2.OperationalError, psycopg2.InterfaceError) as e:
            total_failed += 1
            logger.warning(f"  ❌ {name} DB 連線中斷: {e}，嘗試重連…")
            try:
                conn, cur = reconnect()
                logger.info(f"  重連後重試 {name}…")
                # 重試一次
                cur.execute(sql)
                rows = cur.fetchall()
                if name in PYTHON_FILTERS:
                ...
                conn.commit()
                logger.info(f"  ✅ {name}（重試成功）：{len(symbols)} 支")
                total_success += 1
                total_failed -= 1
